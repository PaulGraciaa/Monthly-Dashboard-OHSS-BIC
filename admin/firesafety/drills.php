...
switch ($action) {
    case 'create':
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($location) || empty($subject)) {
                ...
                $query = "INSERT INTO fire_safety_drills (serial_number, drill_date, location, subject, drill_type, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    case 'delete':
        ...
        $query = "UPDATE fire_safety_drills SET is_active = 0 WHERE id = ?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "i", $id);
...
<head>
    ...
    <title><?= $action == 'create' ? 'Tambah' : 'Edit' ?> Fire Safety Drills</title>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2"><?= $action == 'create' ? 'Tambah' : 'Edit' ?> Fire Safety Drills</h1>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0">
                            ...
                            Form <?= $action == 'create' ? 'Tambah' : 'Edit' ?> Data
                        </h5>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                            <div class="row">
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="serial_number" name="serial_number" 
                                           value="<?= $action == 'edit' ? $data['serial_number'] : (isset($_POST['serial_number']) ? $_POST['serial_number'] : '') ?>" 
                                           required min="1">
                                </div>
                                <div class="col-md-3 mb-3">
                                    <label for="drill_date" class="form-label">Drill Date <span class="text-danger">*</span></label>
                                    <input type="date" class="form-control" id="drill_date" name="drill_date" 
                                           value="<?= $action == 'edit' ? $data['drill_date'] : (isset($_POST['drill_date']) ? $_POST['drill_date'] : '') ?>" 
                                           required>
                                </div>
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-select" id="drill_type" name="drill_type" required>
                                        ...
                                        <option value="drill" <?= ($action == 'edit' && $data['drill_type'] == 'drill') || (isset($_POST['drill_type']) && $_POST['drill_type'] == 'drill') ? 'selected' : '' ?>>Drill</option>
                                        <option value="training" <?= ($action == 'edit' && $data['drill_type'] == 'training') || (isset($_POST['drill_type']) && $_POST['drill_type'] == 'training') ? 'selected' : '' ?>>Training</option>
                                    </select>
                                </div>
                                <div class="col-md-3 mb-3">
                                    <label for="display_order" class="form-label">Display Order</label>
                                    <input type="number" class="form-control" id="display_order" name="display_order" 
                                           value="<?= $action == 'edit' ? $data['display_order'] : (isset($_POST['display_order']) ? $_POST['display_order'] : '0') ?>" 
                                           min="0">
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="location" name="location" 
                                           value="<?= $action == 'edit' ? htmlspecialchars($data['location']) : (isset($_POST['location']) ? htmlspecialchars($_POST['location']) : '') ?>" 
                                           required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="subject" name="subject" 
                                           value="<?= $action == 'edit' ? htmlspecialchars($data['subject']) : (isset($_POST['subject']) ? htmlspecialchars($_POST['subject']) : '') ?>" 
                                           required>
                            ...
                            <div class="mb-3 form-check">
                                <input type="checkbox" class="form-check-input" id="is_active" name="is_active" 
                                       <?= ($action == 'edit' && $data['is_active']) || ($action == 'create' && (!isset($_POST['is_active']) || $_POST['is_active'])) ? 'checked' : '' ?>>
                            ...
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save me-2"></i><?= $action == 'create' ? 'Simpan' : 'Simpan Perubahan' ?>
                                </button>
                            </div>
                        </form>
                    </div>
